...
Unity build, jumbo builds, single compilation unit and such is a build optimization that consists of grouping/merging several CPP files during compilation to build faster.
...
QMake-unity provides good integration with Qt 5 tooling system (qmake, moc, qt-creator).

Here are general articles to better understand unity builds in general :
...
- [Visual studio current support](https://devblogs.microsoft.com/cppblog/support-for-unity-jumbo-files-in-visual-studio-2017-15-8-experimental/)  (not implemented in qmake-unity)

## Installation and usage

This tool is written with Python 3, which must be installed and in the `PATH` environment variable.
...
`MOC_LVL_2` : group MOC invokation of file1.h and file2.h, we build the resulting cpp file
Please note that `MOC_LVL_2` requires "#pragma once" instead of classic include guards

To easily convert your include guards to pragma once, just open a terminal and type :
```
sudo pip3 install guardonce
guard2once.exe -r src
```
You're done !
Run qmake and build, you'll probably have to tune your cpp and headers files. (see sections bellow)

Some cpp/header files are not compatible with unity build, for example headers that rely on inclusion order.
You can remove them from unity by adding the folowing comment in the source : `//@NO_UNITY`, then call qmake again. 
...
After checking that the files that qmake-unity tries to delete are in a tmp directory, you should disable "safe mode" by editing unity_config.py.

* LOG_DEBUG
By default, qmake-unity will print debug logs to help you understand which files are not supported (not grouped).
When your project is ready, you should disable debug logs by editing unity_config.py.
...
You should start with `UNITY_MOC_MODE = MOC_LVL_1`
